...
safe(function(){
  var DATA={"jefe": {"name": "Jefe Burger", "l1": "De 200K à 380K", "l2": "d'abonnés", "stats": [["Abonnés cumulés", "380 K"], ["Likes cumulés", "3,2M"], ["Restaurants", "10"]], "desc": "Création du compte de zéro : identité sociale, rythme de publication hebdomadaire, mise en avant du burger signature et activation des ouvertures. L'enseigne s'est construit une audience avant même d'avoir un réseau national."}, "nst": {"name": "Newschooltacos", "l1": "De 200K à 640K", "l2": "d'abonnés", "stats": [["Abonnés cumulés", "645 K"], ["Likes cumulés", "6,1M"], ["Restaurants", "40"]], "desc": "Reprise complète du compte : nouvelle ligne éditoriale, tournage mensuel dans les restaurants du réseau, formats verticaux natifs et animation quotidienne de la communauté. La marque est passée du statut d'enseigne connue à celui de marque suivie."}, "krousti": {"name": "Krousti Sabaïdi", "l1": "De 70K à 300K", "l2": "d'abonnés", "stats": [["Abonnés cumulés", "308 K"], ["Likes cumulés", "4,2M"], ["Restaurants", "44"]], "desc": "Accompagnement sur douze mois : refonte de la ligne éditoriale, séries récurrentes tournées en cuisine et collaborations créateurs. Le compte est devenu le premier canal d'acquisition du réseau, devant les plateformes de livraison."}, "ta5ty": {"name": "Ta5ty Pizza", "l1": "De 80K à 308K", "l2": "d'abonnés", "stats": [["Abonnés cumulés", "308 K"], ["Likes cumulés", "2M"], ["Restaurants", "21"]], "desc": "Chiffres et description à fournir par le client."}, "bigsmash": {"name": "Big Smash", "l1": "De 50K à 250K", "l2": "d'abonnés", "stats": [["Abonnés cumulés", "250 K"], ["Likes cumulés", "3,2M"], ["Restaurants", "6"]], "desc": "Chiffres et description à fournir par le client."}};
  ...
  /* on affiche l'onglet actif des le chargement : sans ca, le HTML garde
     le contenu fige de la marque d'origine tant qu'on n'a pas clique */
  var first=document.querySelector('.tab.on')||ctabs[0];
  if(first) render(first.dataset.k);
});

/* Le mockup telephone est un WebM VP9 a canal alpha (fond transparent).
   Certains navigateurs LISENT le fichier mais IGNORENT la transparence :
   aucune erreur n'est levee, et le telephone s'affiche sur un rectangle NOIR.
   On ne peut donc pas se fier a canPlayType ni a l'evenement 'error' : on
   verifie la transparence reelle en dessinant une frame sur un canvas et en
   lisant l'alpha d'un pixel de coin. Si le coin est opaque -> image de secours. */
safe(function(){
  var v=document.getElementById('casePhone'), fb=document.getElementById('casePhoneFb');
  if(!v||!fb) return;
  var done=false;
  function fallback(){ if(done)return; done=true; v.hidden=true; fb.hidden=false; }
  ...
  function testAlpha(){
    if(done) return;
    if(!v.videoWidth) return fallback();
    try{
      ...
      x.drawImage(v,0,0);
      /* on echantillonne les 4 coins : sur un WebM alpha ils sont transparents */
      var pts=[[2,2],[c.width-3,2],[2,c.height-3],[c.width-3,c.height-3]];
      var opaque=0;
      for(var i=0;i<pts.length;i++){
        if(x.getImageData(pts[i][0],pts[i][1],1,1).data[3]>250) opaque++;
      }
      if(opaque===pts.length) fallback(); else ok();
    }catch(e){ /* canvas bloque (CORS) : on garde la video */ ok(); }
  }
  v.addEventListener('loadeddata',function(){ setTimeout(testAlpha,120); });

  /* cette video n'a plus 'autoplay' (il forcait le telechargement au chargement
     de la page) : on la demarre quand la section arrive a l'ecran. */
...
/* ---------- reportage presse : vignette, la video ne charge qu'au clic ---------- */
safe(function(){
  var b=document.getElementById('pressBtn'), v=document.getElementById('pressV');
  if(!b||!v) return;
  b.addEventListener('click',function(){
    b.hidden=true; v.hidden=false;
    v.preload='auto'; v.load();
    v.play().catch(function(){});
  });

  /* le logo du media est fourni separement : on masque le cadre s'il manque */
  var lg=document.getElementById('tf1Logo');
  if(lg) lg.addEventListener('error',function(){
    var w=lg.closest('.press__logo'); if(w) w.style.display='none';
  });
});
...
safe(function(){
  ...
  /* lecture seulement quand la vignette est visible.
     arm() n'est appele QUE si la vignette entre a l'ecran : sinon il forcerait
     le telechargement des dizaines de videos des l'ouverture de la page. */
  var io=new IntersectionObserver(function(es){
    es.forEach(function(e){
      if(e.isIntersecting){
        var v=arm(e.target); if(v) v.play().catch(function(){});
      }else{
        var w=e.target.querySelector('video');
        if(w && w.dataset.armed) w.pause();
      }
    });
  },{threshold:.25});
